...
| `403` | The token is valid but lacks the required scope, or the organization's plan does not include API access. | Create a token with the right [scopes](/api-reference/scopes), or upgrade the plan. |
...
| `429` | You have exceeded 120 requests per minute. | Wait the number of seconds in `Retry-After`, then retry. |
